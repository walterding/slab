...
function toString(src, passwd) {
    var txt = CryptoJS.AES.encrypt(src, passwd);
    if (txt) {
        return txt.toString()
    } else {
        return ''
...
function decrypt(src, passwd) {
    var txt = CryptoJS.AES.decrypt(src, passwd);
    if (txt) {
